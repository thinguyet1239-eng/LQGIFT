...
MINI_APP_URL = "https://hoangquockiet.netlify.app"   # ← Thay bằng URL host HTML
ADMIN_ID     = 8766063561
...
# File lưu danh sách user (đồng bộ với Supabase logic nhưng bot cần list user ID)
...
def register_user(user):
    """Lưu user vào file khi họ dùng /start"""
    data = load_users()
    uid  = str(user.id)
    data[uid] = {
        "id":         user.id,
...
async def check_joined(bot, user_id: int) -> list[str]:
    """Trả về list tên nhóm chưa join"""
    not_joined = []
...
def join_keyboard(not_joined: list[str]):
    """Tạo keyboard với nút join từng nhóm + nút kiểm tra lại"""
    buttons = [
        [InlineKeyboardButton(f"➕ Tham gia {c['label']}", url=c["url"])]
        for c in not_joined
    ]
    buttons.append([InlineKeyboardButton("✅ Tôi đã tham gia — Kiểm tra lại", callback_data="check_join")])
...
# ── CALLBACK: kiểm tra lại sau khi join ───────────────────
async def on_check_join(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        ...
        await query.edit_message_text(text, parse_mode="HTML", reply_markup=join_keyboard(not_joined))
    else:
...
async def broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_html(
            "📢 <b>Cách dùng:</b>\n"
            "<code>/broadcast Nội dung thông báo của bạn</code>\n\n"
    ...
    status_msg = await update.message.reply_text(
        f"📤 Đang gửi thông báo tới {total} người dùng..."
    )
    ...
    await status_msg.edit_text(
        f"✅ <b>Đã gửi thông báo xong!</b>\n\n"
        f"👥 Tổng: {total} người\n"
        f"✔️ Thành công: {ok}\n"
        f"❌ Thất bại: {fail} (chặn bot hoặc đã xoá chat)",
        parse_mode="HTML"
    )
